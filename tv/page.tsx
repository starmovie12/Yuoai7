...
export default function TVPage() {
  ...
  useEffect(() => {
    tmdb.getGenres('tv').then((r: any) => setGenres(r.genres)).catch(() => {});
  }, []);

  const fetchShows = useCallback(async (p = 1, reset = false) => {
    if (fetchingRef.current) return;
    fetchingRef.current = true; setLoading(true);
    try {
      let res;
      if (selectedGenres.length > 0) res = await tmdb.discoverByGenre('tv', selectedGenres.join(','), p);
      else res = await tmdb.getPopular('tv', p);
      if (reset) setShows(res.results);
      else setShows((prev: any[]) => [...prev, ...res.results]);
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(
      e => { if (e[0].isIntersecting && hasMore) fetchShows(page + 1); },
      { threshold: 0.1 }
    );
  ...
  return (
    <div className="min-h-screen bg-[#03060f]">
      ...
      <div className="px-4 md:px-8 lg:px-12 pt-24 pb-8">
        <h1 className="text-3xl md:text-5xl font-black tracking-tighter text-white uppercase italic mb-6">📺 TV Shows</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {shows.map((m: any, i: number) => <MovieCard key={`${m.id}-${i}`} movie={{ ...m, media_type: 'tv' }} index={i} />)}
        ...
        <div ref={sentinelRef} className="h-8 flex items-center justify-center mt-4">
          {loading && shows.length > 0 && <Loader2 className="text-red-600 animate-spin" />}
        </div>
      </div>
      <BottomNav />
    </div>
