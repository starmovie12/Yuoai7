        ...
        <div className="relative z-10 flex flex-col md:flex-row gap-8 px-4 md:px-12 pt-20 pb-12">
          <div className="flex-1 max-w-2xl">
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              <span className="text-[10px] font-black px-2 py-0.5 rounded-lg bg-white/10 border border-white/10 text-white uppercase">{type === 'tv' ? 'SERIES' : 'MOVIE'}</span>
            </div>
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-black tracking-tighter uppercase italic text-white mb-4 leading-none">{title}</h1>
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              <button onClick={() => setShowPlayer(true)} className="flex items-center gap-2 px-6 py-3 bg-white text-black rounded-xl font-black hover:scale-105 active:scale-95 transition-all text-sm">
                <Play size={18} fill="currentColor" /> Watch Now
              </button>
              <button onClick={toggleWatchlist} className={`flex items-center gap-2 px-4 py-3 rounded-xl font-bold text-sm transition-all hover:scale-105 ${inWatchlist ? 'bg-red-600 text-white' : 'bg-white/10 border border-white/10 text-white hover:bg-white/20'}`}>
                {inWatchlist ? <CheckCircle size={18} /> : <Heart size={18} />}
                {inWatchlist ? 'Saved' : '+ Watchlist'}
              </button>
              <div className="relative">
                <button onClick={() => setShowShare(!showShare)} className="flex items-center gap-2 px-4 py-3 bg-white/10 border border-white/10 text-white rounded-xl font-bold text-sm hover:bg-white/20 transition-all">
                  <Share2 size={18} />
                </button>
                {showShare && (
            ...
            <div className="flex items-center gap-2">
              ...
              {[1,2,3,4,5].map(star => (
                <button key={star} onClick={() => handleRate(star)} className={`text-xl transition-all hover:scale-125 ${star <= userRating ? 'text-yellow-400' : 'text-gray-600'}`}>★</button>
              ))}
            </div>
          </div>
      ...
      <div className="px-4 md:px-12 mb-6">
        <div className="flex gap-2 border-b border-white/10">
          {(['overview','cast','more'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-3 text-sm font-bold capitalize transition-all border-b-2 -mb-0.5 ${activeTab === tab ? 'border-red-600 text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}>{tab}</button>
          ))}
        </div>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {details.genres?.map((g: any) => (
                <Link key={g.id} href={`/genre/${g.id}`} className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-sm text-gray-300 hover:bg-white/10 transition-all">{g.name}</Link>
              ))}
            </div>
            {director && (
              <div className="flex items-center gap-3 p-4 bg-white/5 rounded-2xl border border-white/5">
                ...
                <div><p className="text-gray-400 text-xs">Director</p><p className="text-white font-bold">{director.name}</p></div>
              </div>
            )}
          </div>
        )}
