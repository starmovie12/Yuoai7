...
export default function SeriesPage() {
  ...
  const handleNextEpisode = () => {
    const totalEp = details?.seasons?.find((s: any) => s.season_number === activeSeason)?.episode_count || 0;
  ...
  const statusColor: Record<string, string> = { 'Returning Series': 'bg-green-600', 'Ended': 'bg-gray-600', 'Cancelled': 'bg-red-800' };
        ...
        <div className="relative z-10 flex flex-col md:flex-row gap-8 px-4 md:px-12 pt-20 pb-12">
          <div className="flex-1 max-w-2xl">
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              <span className={`text-[10px] font-black px-2 py-0.5 rounded-lg text-white ${statusColor[details.status] || 'bg-white/20'}`}>{details.status || 'SERIES'}</span>
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              <button onClick={() => { setActiveSeason(1); setActiveEpisode(1); setShowPlayer(true); }}
      ...
      <div className="px-4 md:px-12 mb-6">
        <div className="flex gap-2 border-b border-white/10 overflow-x-auto no-scrollbar">
          {(['overview','episodes','cast','more'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-shrink-0 px-4 py-3 text-sm font-bold capitalize transition-all border-b-2 -mb-0.5 ${activeTab === tab ? 'border-red-600 text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}>{tab}</button>
          ))}
        </div>
      </div>
